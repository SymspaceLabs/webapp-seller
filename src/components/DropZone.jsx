...
import { H5, Small } from "./Typography"; // Custom typography component

export default function DropZone({
  onChange,
  title = "Drag & drop file here",
  imageSize = "Upload file",
  acceptFormats = { "image/*": [".png", ".jpeg", ".jpg", ".gif"] }  // Default to image formats
}) {
  const onDrop = useCallback((acceptedFiles) => onChange(acceptedFiles), [onChange]);

  const { getRootProps, getInputProps, isDragActive } = useDropzone({
    onDrop,
    maxFiles: 10, // Limit to one file
    multiple: true, 
    accept: acceptFormats,  // Accept formats passed as prop
  });
